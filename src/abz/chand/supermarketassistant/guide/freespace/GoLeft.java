...
public class GoLeft implements Go {
	...
	public GoLeft(int width, int height) {
		this.width = width;
		this.height = height;
	...
	public Pair<Point, Point> getDirection(Mat lines, Point start, Mat rgbMat) {
		...
		for (double x = width-startX; x > 0; x -= startX){
			...
			for (int i = 0; i < lines.cols(); i++){			
				...
				intersects = LineSegmentIntersection.linesIntersect(x1, y1, x2, y2, x, y, x, height);
				if (intersects){
					break;
				}
			}
			...
			if (count >= 4){		
				Core.line(rgbMat, new Point(x, y), new Point(x, height), color, 2);
				directionLine.first.x = x;
				directionLine.first.y = y;
				directionLine.second.x = x;
				directionLine.second.y = height;
